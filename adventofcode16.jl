inpt = """Sue 1: goldfish: 6, trees: 9, akitas: 0
...
perfumes: 1""", "\n")
	items = split(l)
	good[items[1][1:end-1]] = parse(Int, items[2])
end


sueList = Dict{ASCIIString, Dict{ASCIIString, Int64}}()
for line in split(inpt, "\n")
	items = split(line*",")
	sueProps = Dict{ASCIIString, Int64}()
	for i in 3:2:length(items)
		sueProps[items[i][1:end-1]] = parse(Int, items[i+1][1:end-1])
	end
	sueList[items[2][1:end-1]] = sueProps
end

function pred(k, v)
	for (gk, gv) in good
		value = get(v, gk, "forgot")
		if value == "forgot"
			continue
		end
		# part 1
		...
		elseif gk in ["pomeranians", "goldfish"]
			if value >= gv
				return false
			end
		else
