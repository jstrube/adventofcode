inpt = """turn off 660,55 through 986,197
...
function parseInput(line, grid)
	tokens = split(line)
	endpoint = split(tokens[end], ',')
	startpoint = split(tokens[end-2], ',')
	# @printf("%s\t%d,%d\t%d,%d\n", line, parse(Int, startpoint[1]),parse(Int, startpoint[2]), parse(Int, endpoint[1]), parse(Int, endpoint[2]))
	for x in parse(Int, startpoint[1])+1:parse(Int, endpoint[1])+1, y in parse(Int, startpoint[2])+1:parse(Int, endpoint[2])+1
		# println(x, " ", y)
		if tokens[end-3] == "off"
			if grid[x,y]>0 grid[x,y] -= 1 end
		elseif tokens[end-3] == "on"
			grid[x,y] += 1
		elseif tokens[end-3] == "toggle"
			grid[x,y] += 2
		else
			println("Could not identify ", tokens[end-3])
		end
	end
end

grid = [ 0 for x in 1:1000, y in 1:1000 ]
# inpt = "turn on 499,499 through 500,500"
for line in split(inpt, "\n")
	parseInput(line, grid)
end
println(sum(grid))
